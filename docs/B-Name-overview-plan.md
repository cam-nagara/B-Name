...
要望: **「全ページ一覧」を編集ホームにし、その上で** コマ割り / コマ形状変更 / GP ラフ描画 / 縦書きテキスト / フキダシ配置 **ができるようにする**。コマの 3D データはダブルクリックで `panel_NNN.blend` を開いて編集する方式に変更する。
...
└── pages/
    └── NNNN/
        ├── page.json                  # ページメタ (panels リストを含む)
        └── panels/
            ├── panel_NNN.json         # コマメタ
            ├── panel_NNN.blend        # コマ 3D シーン (ダブルクリックで開く)
            └── panel_NNN_thumb.png    # サムネ
```
...
| **panel 編集モード** | `panel_NNN.blend` | そのコマの 3D シーン (アセットリンク等) |

モード切替は **`wm.open_mainfile`** で mainfile 自体を差替える。

### Scene プロパティ

- `Scene.bname_work` (PointerProperty) — work.blend 内で永続。`work.pages[i].panels[j]` に全ページ・全コマのメタを保持
...
[起動]
  └─ work.blend を open → overview モード
       ├─ コマダブルクリック → work.blend save → panel_NNN.blend open → panel モード
       │     └─ Exit → panel.blend save → work.blend open → overview モード
...
### 範囲内
- `io/blend_io.py`: `save_work_blend` / `open_work_blend` を追加。`save_page_blend` / `open_page_blend` は削除。panel 系は維持
- `operators/work_op.py`:
  ...
  - `work_save`: 現在 mainfile が work.blend なら save_mainfile、panel.blend なら save_as_mainfile に委譲
- `operators/page_op.py`:
  ...
  - `page_remove`: ページ Collection を削除、page.json/panel_NNN.json を削除。mainfile 差替えロジック廃止
  - `page_select` / `_switch_to_page`: page.blend swap を削除。active_page_index の更新と viewport フィットのみ
- `operators/mode_op.py`:
  - `enter_panel_mode`: save work.blend (save_mainfile) → open panel_NNN.blend。未作成なら現シーンを save_as
  - `exit_panel_mode`: save panel.blend → open work.blend
- `utils/handlers.py`:
  - `_on_load_post`: `work.blend` ロード時は work_dir/pages.json から全メタを Scene に復元。`panel_NNN.blend` ロード時は mode=PANEL + 該当 panel_stem を設定
  - `_sync_active_from_blend_path` を `work.blend` / `panel_NNN.blend` の 2 系統に対応 (page.blend 分岐を削除)
- `ui/overlay.py`:
...
- **コマ一覧 UIList にダブルクリック → `bname.enter_panel_mode`** をハンドラで結線:
  ...
  - ビューポート上のパネルダブルクリックは keymap で `LEFTMOUSE` `DOUBLE_CLICK` を `bname.enter_panel_mode` に結線、invoke 時に world 座標→対象パネル逆引き
- ショートカット keymap に `LEFTMOUSE` `DOUBLE_CLICK` を `bname.enter_panel_mode` 割当 (「B-Name Viewport」keymap、既存 watcher 制御に従う)
...
- [ ] Phase 1 の コマ操作 / panel 編集遷移は壊れていない
